...
def create_model( session, actions, batch_size ):
  ...
  if ckpt and ckpt.model_checkpoint_path:
    ...
    if FLAGS.load > 0:
      ...
      if os.path.isfile(Path(train_dir)/"checkpoint-{0}.index".format(FLAGS.load)):
        ckpt_name = os.path.join( os.path.join(train_dir,"checkpoint-{0}".format(FLAGS.load)) )
      else:
        raise ValueError("Asked to load checkpoint {0}, but it does not seem to exist".format(os.path.join(train_dir,"checkpoint-{0}.index".format(FLAGS.load))))
    else:
